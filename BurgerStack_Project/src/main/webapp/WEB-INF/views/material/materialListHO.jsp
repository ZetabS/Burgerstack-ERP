        ...
        <div class="all-list">
            ...
            <div class="category-section">
                <c:forEach var="m" items="${materials}" varStatus="status">
                    ...
                    <div class="img-wrap" onclick="MaterialDrawer.getDetail('${m.materialId}', '${m.materialName}')">
    ...
    <script>
        ...
        const MaterialDrawer = {
            ...
            bindData: function(data) {
                ...
                if (imgEl) {
                    const srcName = data.imageFileId ? data.imageFileId : 'BS_logo1.svg';
                    imgEl.src = this.contextPath + "/resources/images/" + srcName;
            ...
            bindEvents: function(materialId) {
                ...
                if (deleteBtn) {
                    deleteBtn.onclick = () => {
                        this.deleteMaterial(materialId); // 💡 삭제 함수 호출
                    };
                }
            },
            ...
            /**
             * 💡 [추가된 삭제 실행 메서드]
             * confirm창을 띄우고 확인 시 POST로 안전하게 컨트롤러 전송
             */
            deleteMaterial: function(materialId) {
                if (confirm("정말 삭제하시겠습니까?")) {
                    // 가상 폼 생성을 통한 안전한 POST 전송
                    const form = document.createElement('form');
                    form.method = 'POST';
                    
                    // 💡 백틱(`) 대신 홑따옴표와 + 연산자를 사용하여 안전하게 주소를 조립합니다.

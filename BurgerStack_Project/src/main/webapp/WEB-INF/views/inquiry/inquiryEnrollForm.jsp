...
<c:url var="listUrl" value="/owner/dashboard" />
<c:url var="submitUrl" value="/owner/inquiries/new" />
      ...
      <form action="${submitUrl}" method="post">
        ...
        <layout:Section title="문의사항을 작성해주세요." description="">
          ...
          <layout:FieldRow label="제목" inputId="safetyQuantity" help="제목을 입력하시오.">
            <input type="text" class="title" id="title" name="title" value="${inquiry.title}" required />
          </layout:FieldRow>

          <%-- textarea는 업무에 맞게 rows를 직접 선택합니다. --%>
          <layout:FieldRow label="내용" inputId="memo" help="문의사항을 입력하시오.">
            <textarea class="form-control" id="content" name="content" rows="4"><c:out value="${form.memo}" /></textarea>
          </layout:FieldRow>
        </layout:Section>
